...
class CAGNN(nn.Module):
    ...
    def forward(self, g: dgl.DGLHeteroGraph, seq_len, seq_len_uniq, u_input, category_ids, alias_inputs, adj,
        ...
        cos_sim = nn.Softmax(dim=-1)(cos_sim)  # zz 5128'512
...
def train_test(model, train_data, test_data):
    ...
    model.train()
    total_loss = 0.0
    train_loader = DataLoader(train_data, batch_size = model.batch,shuffle=True,num_workers=4, collate_fn=collate_fn, pin_memory = True)
    ...
    model.eval()
    test_loader = DataLoader(test_data,num_workers=4, batch_size=model.batch, shuffle=False, collate_fn=collate_fn)
